...
module.exports = function getOauthAccessToken(config, logger, code, state) {
  ...
  assert.equal(typeof state, 'string', 'required argument state must be a string');

  // default values
  config.url = config.url || 'https://www.linkedin.com/uas/oauth2/authorization';

  var accessTokenParameters = buildAccessTokenParameters(config, code, state);
...
function buildAccessTokenParameters(config, code, state) {
